...
class MaskTo2DMesh:
    ...
    def __init__(
        ...
        Args:
        boundary_resolution (int): 在平滑边界上重新采样的顶点数量。
        smooth_factor (Optional[float]): B样条插值的平滑因子 's'。
                                        值越大，曲线越平滑但可能偏离原始轮廓。
                                        'None' 将让scipy自动选择 (m - sqrt(2*m))。
                                        通常 0.1 - 5.0 是一个合理的调参范围。
        mesh_max_area (float): 'triangle'库的 'a' 参数，控制最大三角形面积（网格密度）。
                               值越小，网格越精细。
        mesh_min_angle (float): 'triangle'库的 'q' 参数，控制最小三角形角度（网格质量）。
        """
    ...
    def _generate_triangle_mesh(self, smoothed_contour: np.ndarray) -> Dict[str, Any]:
        ...
        Args:
            smoothed_contour (np.ndarray): 平滑后的轮廓点数组 (M, 2)
        """
        ...
        # q: 质量约束 (最小角度)
        # a: 面积约束 (最大面积)
        opts = f"pq{self.mesh_min_angle}a{self.mesh_max_area}n"
    ...
    def generate_mesh(self, mask_image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray]:
                ...
                F (np.ndarray): 三角形面片索引 (F_num, 3), 0-based
        """
        # 模块 1: 提取
        print("[M1] 正在提取轮廓...")
        ...
        self.neighbors = mesh_data.get('neighbors', None)  # 形状为 (N, 3)
        ...
        print(f"     ... 生成了 {len(V)} 个顶点和 {len(F)} 个三角形面片")
        print(f"     ... 平均边长为 {self.L_avg:.6f}")
    ...
    def build_face_laplacian(triangles):
        num_faces = len(triangles)
        
        # 存储 边 -> 面索引 的映射
        # 边用排序后的元组 (v_min, v_max) 表示
        edge_to_faces = {}
        
        for f_idx, nodes in enumerate(triangles):
            # 提取三角形的三条边
            edges = [
        ...
        # 每个相邻对贡献权重 1
        W = np.ones(len(I))
        adj = sparse.csr_matrix((W, (I, J)), shape=(num_faces, num_faces))

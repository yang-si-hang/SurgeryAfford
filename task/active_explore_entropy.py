"""
基于Entropy最大化的主动刚度估计任务规划器:
loss = - log det(Y_prior + H^T R^-1 H)

约束条件:
1. 动作幅值约束: ||u||_2 <= max_mag
2. 应变约束: 应变分量约束在[sig_min, sig_max]范围内

优化器: IPOPT / SciPy SLSQP / SciPy trust-constr
IPOPT的稳定性高
SLSQP速度快, 依赖初始值
trust-constr

都存在一个问题, 没有办法探索多个方向, 甚至有时候不能收敛到合理解.
...
class ActiveStiffnessPlanner:
    ...
    def visualize_loss_landscape(self, dH_du, H0, R_inv, Y_prior, max_mag=0.03):
        """
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                # --- 前向计算 (与 objective_func 逻辑一致) ---
                H_delta = torch.einsum('ijk,k->ij', dH_du, u_tensor)
                H_pred = H0 + H_delta
                
                # Y_new = Y_prior + H^T R^-1 H
                term = R_inv @ H_pred
                info_gain = H_pred.T @ term
                Y_new = Y_prior + info_gain
                
                # Loss = -logdet
                sign, logdet = torch.linalg.slogdet(Y_new)
                loss = -logdet
    ...
    def optimize_action(self, solver_method: str, 
                        contact_node_idx: int, 
                        current_q_vision: torch.Tensor,
                        current_stiffness_est: torch.Tensor,
                        max_action_mag: float = 0.03,
                        iter_num: int = 100,
                        u0_np: np.ndarray = np.zeros(2)) -> np.ndarray:
        """
        基于当前视觉观测和刚度估计，优化下一步的施力动作
        Args:
            contact_node_idx: 施力点索引
            current_q_vision: 视觉测量的当前位置 (Tensor)
            current_stiffness_est: 当前刚度估计 (Tensor)
            max_action_mag: 动作幅值的截断上限
            iter_num: 优化迭代次数
            u0_np (np.ndarray): 初始动作猜测
        Returns:
            optimal_action: 优化后的动作向量
        """
        # 更新物理模型到当前观测状态
        ...
        # 获取基准状态下的梯度矩阵
        obs_dofs = self.estimator.obs_dofs
        H_base, K_T_base = self.soft_model.get_gradients_at_state(obs_dofs)
        H0 = H_base.clone()
        ...
        # 构造观测噪声协方差 R 的逆 (信息加权矩阵), R = J_f * Sigma_q * J_f^T + Sigma_f
        Sigma_q_mat = self.estimator.sigma_q_val * torch.eye(self.estimator.OBS_NUM*2, device=self.device, dtype=torch.float64)
        sigma_f_full = torch.zeros(self.soft_model.PARTICLE_N*2, device=self.device, dtype=torch.float64)
        sigma_f_full[contact_node_idx*2 : contact_node_idx*2+2] = self.estimator.sigma_f_val  # 仅在接触点有力传感器噪声
        self.Sigma_f_mat = torch.diag(sigma_f_full[obs_dofs])

        J_f_obs = K_T_base[obs_dofs, :][:, obs_dofs]
        term1 = J_f_obs @ Sigma_q_mat @ J_f_obs.T
        R_k = term1 + self.Sigma_f_mat
        R_k_inv = torch.linalg.inv(R_k)

        # print(f"--- Optimizing Action for Node {contact_node_idx} ---")
        
        P_prior = self.estimator.P
        # 添加微小正则项防止奇异
        Y_prior = torch.linalg.inv(P_prior + torch.eye(P_prior.shape[0], device=self.device, dtype=torch.float64) * 1e-8)
        ...
        sign, logdet = torch.linalg.slogdet(Y_prior)
        loss_current = -logdet.cpu().item()
        # print(f"Current -logdet(Y_prior): {loss_current:.4f}")
        
        # print(f"--- Optimizing Action for Node {contact_node_idx} (D-Optimality) ---")

        # ======================================================================
        # SciPy Objective Function for D-Optimality
        # Maximize log(det(Y_new))  =>  Minimize -log(det(Y_new))
        # ======================================================================
        def objective_func(u_vec:np.ndarray):
            u_tensor = torch.from_numpy(u_vec).to(self.device, dtype=torch.float64)
            
            # 预测 H: H(u) = H0 + dH/du * u
            # 使用 einsum 进行张量乘法: (N_obs, N_elem, N_act) @ (N_act) -> (N_obs, N_elem)
            H_delta = torch.einsum('ijk,k->ij', dH_du, u_tensor)    # * current_stiffness_est.repeat(len(obs_dofs), 1)
            H_pred = H0 + H_delta
            # np.savetxt(OUTPUT_DIR / f"H_pred_einsum_u_{u_vec[0]:.2f}_{u_vec[1]:.2f}.csv", H_pred.cpu().numpy(), delimiter=",")
            
            # 计算后验信息矩阵 Y_new = Y_prior + H^T * R^-1 * H
            term = R_k_inv @ H_pred
            info_gain = H_pred.T @ term     # info_gain = H^T * R^-1 * H
            Y_new = Y_prior + info_gain
            # np.savetxt(OUTPUT_DIR / f"H_pred_base_u_{u_vec[0]:.2f}_{u_vec[1]:.2f}.csv", H_pred_.cpu().numpy(), delimiter=",")
            # np.savetxt(OUTPUT_DIR / f"H_pred_u_{u_vec[0]:.2f}_{u_vec[1]:.2f}.csv", H_pred.cpu().numpy(), delimiter=",")
            # np.savetxt(OUTPUT_DIR / f"info_gain_u_{u_vec[0]:.2f}_{u_vec[1]:.2f}.csv", info_gain.cpu().numpy(), delimiter=",")
            
            # 4. 计算 Loss: -log(det(Y_new))
            sign, logdet = torch.linalg.slogdet(Y_new)  # 返回 (sign, logabsdet)
            
            # 如果行列式非正(数值问题)，返回大值惩罚
            if sign.item() <= 0:
                return 1e9, np.zeros_like(u_vec)
                
            loss = -logdet.cpu().item()

            # 5. 计算梯度
            # d(-logdet)/du = - Tr( Y_new^-1 * d(Y_new)/du )
            # d(Y_new)/du = d(H^T R^-1 H)/du
            # 最终导出的梯度公式: Grad_H = 2 * R^-1 * H * P_new
            # 其中 P_new = Y_new^-1
            
            P_new = torch.linalg.inv(Y_new) # 如果 N_elem 很大 (>500)，这里会成为瓶颈
            Grad_H = 2.0 * (term @ P_new) # (N_obs, N_elem), term is R^-1 * H
            
            # 投影回 u: Grad_u = Grad_H : dH/du
            # (N_obs, N_elem) : (N_obs, N_elem, N_act) -> (N_act)
            grad_u_tensor = -torch.einsum('ij,ijk->k', Grad_H, dH_du)

            # print(f"Action u: {u_vec} ({np.linalg.norm(u_vec):.4f}) => -logdet(Y_new): {loss:.4f}(Δ: {loss - loss_current:.4f})")#; {grad_u_tensor.cpu().numpy()}")
            
            # 将 loss 压缩到 1 附近
            return loss / loss_current, grad_u_tensor.cpu().numpy() / loss_current

        # ===== 动作约束: ||u|| <= max_action_mag =====
        def compute_act_cons(u):
            val = np.dot(u, u) / (max_action_mag**2 + 1e-12)
            # 约束形式: 1.0 - |u|^2 >= 0
            if val >= 1.0 + 1.e-3:
                pass
                # print(f"[Action Constraint Violation] u={u}, |u|={np.sqrt(val)*(max_action_mag):.4f} > {max_action_mag:.4f}")
            return 1.0 - val
        
        def compute_act_jac(u):
            # d(1-u^2)/du = -2u
            return -2*u / (max_action_mag**2 + 1e-12)
        ...
        # ===== 约束应变的大小: sig_i ∈ [limit_min, limit_max] =====
        def compute_strain(u_vec:torch.Tensor, dF_du:torch.Tensor):
            """
            计算约束值和 Jacobian
            Args:
                u_vec: (N_act, ) 当前动作
            ...
            """
            # 预测变形梯度 F (N_elem, 2, 2)
            F_base = torch.eye(2).repeat(dF_du.shape[0], 1, 1).to(self.device, dtype=torch.float64)  # 基准 F 矩阵 (单位矩阵)
        ...
        def compute_strain_constraints(u_np:np.ndarray):
            u_tensor = torch.from_numpy(u_np).to(self.device, dtype=torch.float64)

            # 计算所有单元的奇异值 S (N_elem, 2); S[:, 0] is max, S[:, 1] is min
            S, dSig0_du, dSig1_du = compute_strain(u_tensor, dF_du_tensor)
            
            rho = 200.0 # 平滑因子
            
            # Max Constraint (SoftMax)
            limit_max = 6. # 允许最大拉伸 1.6 倍
            sig_max_vals = S[:, 0]
            
            # LogSumExp Trick
            m_val = torch.max(sig_max_vals)
            # sum( exp( rho * (val - max) ) )
            lse_max = m_val + (1/rho) * torch.log(torch.sum(torch.exp(rho * (sig_max_vals - m_val))))
            
            val_stretch = -(lse_max - limit_max).item()
            
            # Gradient: Weighted sum
            weights_max = torch.softmax(rho * sig_max_vals, dim=0) # PyTorch自带稳定的softmax
            grad_stretch = torch.sum(weights_max.unsqueeze(1) * dSig0_du, dim=0).cpu().numpy()
            
            # Min Constraint (SoftMin)
            rho = 200.0 # 平滑因子
            limit_min = 0.6 # 允许最小压缩到 0.8 倍
            sig_min_vals = S[:, 1]
            
            # SoftMin = -SoftMax(-x)
            neg_vals = -sig_min_vals
            m_neg = torch.max(neg_vals)
            lse_min_neg = m_neg + (1/rho) * torch.log(torch.sum(torch.exp(rho * (neg_vals - m_neg))))
            soft_min = -lse_min_neg
            
            val_compress = -(limit_min - soft_min).item()
            
            # Gradient
            # d(SoftMin)/du = sum( softmax(-rho*x)_i * d(x_i)/du )
            weights_min = torch.softmax(-rho * sig_min_vals, dim=0)
            # 注意符号: 约束是 L - SoftMin <= 0, 所以梯度是 -d(SoftMin)/du
            grad_compress = -torch.sum(weights_min.unsqueeze(1) * dSig1_du, dim=0).cpu().numpy()

            if val_stretch < 1.e-6:
                max_val, max_idx = torch.max(sig_max_vals, dim=0)
                # print(f"[Stretch Violation] u={u_np}, Stretch={val_stretch:.4f}, Max Sig: {max_val.cpu().numpy():.4f} ({max_idx.item()})")
            elif val_compress < 1.e-6:
                min_val, min_idx = torch.min(sig_min_vals, dim=0)
                # print(f"[Compress Violation] u={u_np}, Compress={val_compress:.4f}, Min Sig: {min_val.cpu().numpy():.4f} ({min_idx.item()})")
            
            final_val = np.array([val_stretch, val_compress])   # Values: (2,)
            final_jac = np.vstack([grad_stretch, grad_compress])    # Jacobian: (2, N_act) -> Stack rows
            
            return val_stretch, val_compress, grad_stretch, grad_compress
        ...
        def update_cache(u):
            if strain_cache['last_u'] is None or not np.allclose(u, strain_cache['last_u'], atol=1e-8):
                v1, v2, g1, g2 = compute_strain_constraints(u)
                strain_cache['last_u'] = np.copy(u)
                strain_cache['val'] = np.array([v1, v2])
                strain_cache['jac'] = np.vstack([g1, g2])

        # 定义两个独立的约束字典 (IPOPT 不支持返回向量的单个约束函数，最好拆开)
        cons_strain_stretch = {
            'type': 'ineq',
            'fun': lambda u: (update_cache(u), strain_cache['val'][0])[1],
            'jac': lambda u: (update_cache(u), strain_cache['jac'][0])[1]
        }

        cons_strain_compress = {
            'type': 'ineq',
            'fun': lambda u: (update_cache(u), strain_cache['val'][1])[1],
            'jac': lambda u: (update_cache(u), strain_cache['jac'][1])[1]
        }

        # 组装约束列表
        constraints = [cons_act, cons_strain_stretch, cons_strain_compress]
        mag_float = float(max_action_mag)
        bounds_list = [(-mag_float, mag_float), (-mag_float, mag_float)]
        
        # ===== 调用 IPOPT / SLSQP / trust-constr 优化器 =====
        start_time = time.time()
        if solver_method.lower() == 'ipopt':
            # IPOPT 配置
            ipopt_options = {
                'print_level': 0,
                'max_iter': iter_num,
        ...
        elif solver_method == 'trust-constr':
            # SciPy trust-constr 配置
            # 注意: trust-constr 虽然也支持字典约束，但通常用 NonlinearConstraint 对象效果更好
            # 这里为了兼容，直接传入字典列表也是可以的
            scipy_options = {
                'maxiter': iter_num,
                'gtol': 1e-4,
                'xtol': 1e-4,
                'verbose': 2 # 1=summary, 2=detailed
            }
        ...
        node_pos_new = current_q_vision + (dq_du @ torch.from_numpy(res.x).to(self.device)).view(current_q_vision.shape)
...
if __name__ == "__main__":
    ...
    P_prior = ekf.P
    # 添加微小正则项防止奇异
    Y_prior = torch.linalg.inv(P_prior + torch.eye(P_prior.shape[0], device="cuda", dtype=torch.float64) * 1e-8)

    sign, logdet = torch.linalg.slogdet(Y_prior)
    loss_current = -logdet.cpu().item()
    print(f"Intial -logdet(Y_prior): {loss_current:.4f}")
    ...
    # candidate_contact = list(range(1, 45))
    ...
    for contact_idx in candidate_contact:
        ...
        optimal_action, optimal_fun = planner.optimize_action(
            solver_method='ipopt',   # SLSQP, trust-constr, ipopt
            contact_node_idx=contact_idx,
            current_q_vision=vision_q_tensor,
            current_stiffness_est=torch.ones(FACE_NUM, device="cuda") * 400.0,
            max_action_mag=0.06,
            iter_num=500,
            u0_np = np.array([0.0, -0.0]),
        )
        ...
        print(f"Optimal Objective Function Value: {optimal_fun:.4f}; (Reduction: {increment:.4f})")
    ...
    soft_init_model = Soft2DForceExtended(
                shape=MESH_DATA, fix=FIXED_NODES, 
                contact=37, E=1.e1, nu=0.3, dt=1.e-2, density=1.e1, device="cuda",
            )
    ...
    time_start = time.time()
    optimal_action, optimal_fun = planner.optimize_action(
        solver_method='SLSQP',   # SLSQP, trust-constr, ipopt
        contact_node_idx=37,
        current_q_vision=vision_q_tensor,
        current_stiffness_est=torch.ones(FACE_NUM, device="cuda") * 400.0,
        iter_num=100,
        u0_np = np.array([0.01, -0.01]),
    )
    spend_time = time.time() - time_start
    print(f"Optimization Total Time: {spend_time:.4f} seconds")
    print(f"Optimal Action: {optimal_action}; Magnitude: {np.linalg.norm(optimal_action):.4f}")
    print(f"Optimal Entropy: {optimal_fun:.4f}")
...
class Soft2D:
    ...
    def model_gradient(self):
        """ 计算变形梯度雅可比矩阵dq/dy中与contact相关的列 """
        mass_np = self.node_mass.to_numpy() / self.dt**2
        mass_matrix = np.diag(np.repeat(mass_np, 2))
        A = mass_matrix + np.kron(self.lhs_stretch.to_numpy(), np.eye(2)) + \
            np.kron(self.lhs_positional.to_numpy(), np.eye(2)) - self.dA
        B = np.diag(self.dx_const.to_numpy())
        time_start = time.time()
        # 只需要与contact相关的列，减少计算量
        contact_idx = self.contact_particle_list[0]
        # dq_dy_np = np.linalg.solve(A, B)
        dq_dy_np = np.linalg.solve(A, B[:,2*contact_idx:2*contact_idx+2])    # deformation gradient雅可比矩阵
        print(f"Time taken: {time.time() - time_start}")
        # np.savetxt("A.csv", A, fmt="%.6f", delimiter=",")
        # np.savetxt("B.csv", B, fmt="%.6f", delimiter=",")
        # np.savetxt("dq_dy.csv", dq_dy_np, fmt="%.6f", delimiter=",")
        return dq_dy_np
    ...
    def compute_task_afford(self):
        ...
        dq_dcontact = self.model_gradient()
        afford = self.compute_afford(dq_dcontact)
        return dq_dcontact, afford


if __name__ == "__main__":
    mesh_file = ROOT_DIR / "output_mesh.msh"
    fix_list = list(range(0, 161, 16))
    contact_idx = 15
    # soft_model = Soft2D(shape=[0.15, 0.1], fix=fix_list, contact=contact_idx,
    #                           E=1.e3, nu=0.3, dt=1.e-2, density=1.e1)
    soft_model = Soft2D(shape=[150, 100], fix=fix_list, contact=contact_idx,
                              E=1.e3, nu=0.3, dt=1.e-2, density=1.e1)
